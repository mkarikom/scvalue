...
def get_parser(parser=None):
    ...
    parser.add_argument("-rs", '--random_seed', type=int, help="Random seed for training (default: 42)", default=42)
    parser.add_argument("-sm", "--sketch_method", type=str, help="Sketch method (default: scoob)", default='scoob')
    parser.add_argument("-ss", "--sketch_size", type=int, help="Sketch size (default: 3000)", default=3000)
    return parser
...
def scale_sets(sets):
    ...
    print('Common genes num = %d' % (len(common_genes)))
    sep_point = [0]
    ...
    total_set = np.divide(total_set, np.sum(total_set, axis=0, keepdims=True)+0.000001) * 10000
    total_set = np.log2(total_set+1)
    expr = np.sum(total_set, axis=1)
    total_set = total_set[np.logical_and(expr >= np.percentile(expr, 1), expr <= np.percentile(expr, 99)),]
    cv = np.std(total_set, axis=1) / (np.mean(total_set, axis=1) + 0.000001)
...
# Build the model
def model(X_train, Y_train, X_test, seed, starting_learning_rate = 0.0001, num_epochs = 1500, minibatch_size = 128, print_cost = True):
...
if __name__ == '__main__':
    ...
    train_adata = sc.read_h5ad(args.train_set)
    train_adata.X = train_adata.X.toarray()
    train_set = pd.DataFrame(train_adata.X.T.copy(), columns=train_adata.obs_names, index=train_adata.var_names)
    train_set.index = [s.upper() for s in train_set.index]
    train_set = train_set.loc[~train_set.index.duplicated(keep='first')]
    train_label = train_adata.obs[['free_annotation']].copy()
    
    #test_set = pd.read_hdf(args.test_set, key="dge")
    test_adata = sc.read_h5ad(args.test_set)
    test_adata.X = test_adata.X.toarray()
    test_set = pd.DataFrame(test_adata.X.T.copy(), columns=test_adata.obs_names, index=test_adata.var_names)
    ...
    nt = len(set(train_label.iloc[:,0]))
    train_set, test_set = scale_sets([train_set, test_set])
    type_to_label_dict = type_to_label_dict(train_label.iloc[:,0])
    ...
    train_label = convert_type_to_label(train_label.iloc[:,0], type_to_label_dict)
    train_label = one_hot_matrix(train_label, nt)
    parameters = model(train_set, train_label, test_set, seed, \
    ...
    os.makedirs("experiments/seed%d" % (args.random_seed), exist_ok=True)
    predicted_label.to_csv("experiments/seed%d/predicted_label.%s.%d.txt" % (args.random_seed, args.sketch_method, args.sketch_size), sep="\t", index=False)
    #predicted_label.to_csv("experiments/predicted_label.txt", sep="\t", index=False)
